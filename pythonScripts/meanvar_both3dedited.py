...
def CreateCoProcessor():
  def _CreatePipeline(coprocessor, datadescription):
    class Pipeline:
      ...
      data = src.GetPointDataInformation()
      print(data)
    #data = source.GetPointDataInformation()RescaleDataRange

    #get the array and the respective min-max
      array = data.GetArray('rho_mean')
      dataRange = array.GetRange(0)
      rho_meanLUT = GetColorTransferFunction('rho_mean')
    #  rho_meanLUT.AutomaticRescaleRangeMode = 1

    #Now let's hack in the range values
      rho_meanLUT.RGBPoints[0] = dataRange[0] #min
      rho_meanLUT.RGBPoints[4] = dataRange[1] #max
